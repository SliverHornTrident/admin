...
func newRole(db *gorm.DB, opts ...gen.DOOption) role {
	...
	_role.IsDelete = field.NewUint(tableName, "is_delete")
	_role.Name = field.NewString(tableName, "name")
	_role.Menu = field.NewString(tableName, "menu")
	_role.ParentId = field.NewUint(tableName, "parent_id")
	_role.Overstep = field.NewBool(tableName, "overstep")
	_role.Menus = roleManyToManyMenus{
		...
		Buttons: struct {
			...
			RelationField: field.NewRelation("Menus.Buttons", "model.MenuButton"),
			Menu: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Menus.Buttons.Menu", "model.Menu"),
			},
			Apis: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Menus.Buttons.Apis", "model.Api"),
			},
		},
		Parameters: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Menus.Parameters", "model.MenuParameter"),
		},
		Apis: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Menus.Apis", "model.Api"),
		},
	}

	_role.MenuButtons = roleManyToManyMenuButtons{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("MenuButtons", "model.MenuButton"),
	}
...
type role struct {
	...
	MenuButtons roleManyToManyMenuButtons
...
func (r *role) updateTableName(table string) *role {
	...
	r.IsDelete = field.NewUint(table, "is_delete")
	r.Name = field.NewString(table, "name")
	r.Menu = field.NewString(table, "menu")
	r.ParentId = field.NewUint(table, "parent_id")
	r.Overstep = field.NewBool(table, "overstep")
...
func (r *role) fillFieldMap() {
	...
	r.fieldMap["menu"] = r.Menu
	r.fieldMap["parent_id"] = r.ParentId
	r.fieldMap["overstep"] = r.Overstep
...
type roleManyToManyMenuButtons struct {
	db *gorm.DB

	field.RelationField
}

func (a roleManyToManyMenuButtons) Where(conds ...field.Expr) *roleManyToManyMenuButtons {
	if len(conds) == 0 {
		return &a
	}
...
func (a roleManyToManyMenuButtons) WithContext(ctx context.Context) *roleManyToManyMenuButtons {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a roleManyToManyMenuButtons) Session(session *gorm.Session) *roleManyToManyMenuButtons {
	a.db = a.db.Session(session)
	return &a
}

func (a roleManyToManyMenuButtons) Model(m *model.Role) *roleManyToManyMenuButtonsTx {
	return &roleManyToManyMenuButtonsTx{a.db.Model(m).Association(a.Name())}
}

type roleManyToManyMenuButtonsTx struct{ tx *gorm.Association }

func (a roleManyToManyMenuButtonsTx) Find() (result []*model.MenuButton, err error) {
	return result, a.tx.Find(&result)
}

func (a roleManyToManyMenuButtonsTx) Append(values ...*model.MenuButton) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyMenuButtonsTx) Replace(values ...*model.MenuButton) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyMenuButtonsTx) Delete(values ...*model.MenuButton) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyMenuButtonsTx) Clear() error {
	return a.tx.Clear()
}

func (a roleManyToManyMenuButtonsTx) Count() int64 {
	return a.tx.Count()
}
