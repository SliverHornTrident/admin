...
func (s *engine) Name() string {
	return "[shadow][core][gin][engine][linux][embed]"
}

func (s *engine) Viper(viper *viper.Viper) error {
...
func (s *engine) IsPanic() bool {
	return true
}

func (s *engine) ConfigName() string {
	return strings.Join([]string{"gin", gin.Mode(), "yaml"}, ".")
}

func (s *engine) Initialization(ctx context.Context) error {
